...
async def apply_revision(request: RevisionRequest) -> RevisionResponse:
    ...
    try:
        ...
        # Deterministic re-verification: the revision may have introduced new
        # statute citations. Annotate per settings; never fail the revision.
        citation_verification = None
        try:
            from app.engine.citation_verifier import run_citation_verification

            ao = dict(request.appeal_output or {})
            ao["appeal_letter"] = revised
            ao.setdefault("letter_type", "payer_appeal")
            ao.setdefault("cited_regulations", [])
            ao.setdefault("policy_citations", [])
            citation_verification, checked = run_citation_verification(
                appeal_output=ao, knowledge_chunks=[], policy_chunks=[]
            )
            revised = str(checked.get("appeal_letter") or revised)
        except Exception as e:  # pragma: no cover - QA must never break a revision
            logger.warning(f"Studio: citation re-verification skipped: {e}")
...
def compile_dossier(evidence: dict) -> DossierPackage:
    ...
    cover = {
        ...
        # Deterministic score + grounding (additive; None when the appeal_qa node did not run)
        "success_score": ao.get("success_score"),
        "success_score_band": ao.get("success_score_band"),
        "citation_grounding_score": (ao.get("citation_verification") or {}).get("grounding_score"),
        "citation_status": (ao.get("citation_verification") or {}).get("status"),
