...
class CopaySchedule(BaseModel):
    ...
    primary_care: float | None = Field(None, description="PCP office visit copay")
    specialist: float | None = Field(None, description="Specialist visit copay")
    urgent_care: float | None = Field(None, description="Urgent care visit copay")
    emergency_room: float | None = Field(None, description="ER visit copay (may be waived if admitted)")
    generic_rx: float | None = Field(None, description="Tier 1 generic drug copay")
    preferred_brand_rx: float | None = Field(None, description="Tier 3 preferred brand drug copay")
    specialty_rx: float | None = Field(None, description="Tier 5 specialty drug copay/coinsurance")
...
class PolicyProfile(BaseModel):
    ...
    plan_type: PlanType | None = Field(None, description="Network model: HMO, PPO, EPO, or POS")
    legal_classification: PlanLegalClassification | None = Field(
        None, description="Legal classification — determines appeal framework"
    )
    ...
    in_network_deductible_individual: float | None = Field(
        None, ge=0, description="Annual in-network individual deductible"
    )
    in_network_deductible_family: float | None = Field(
        None, ge=0, description="Annual in-network family deductible"
    )
    in_network_oop_max_individual: float | None = Field(
        None, ge=0, description="Annual in-network individual out-of-pocket maximum"
    )
    in_network_oop_max_family: float | None = Field(
        None, ge=0, description="Annual in-network family out-of-pocket maximum"
    )
    in_network_coinsurance: float | None = Field(
        None, ge=0, le=1, description="Patient coinsurance rate (e.g., 0.20 for 80/20 plan)"
    )
    ...
    # ── Plan Features ─────────────────────────────────────────────
    is_hsa_eligible: bool | None = Field(None, description="True if this is an HDHP eligible for HSA")
    requires_pcp_referral: bool | None = Field(
        None, description="True for HMO and POS — specialist visits require PCP referral"
    )
